...
def train(cfg, model, emb_weights, optimizer, learning_rate, global_step, train_ds, strategy, lfw_evaluator):
    ...
    ckpt_manager, ckpt = load_ckpt(cfg['ckpt_dir'], cfg['max_to_keep'], cfg['backbone'], global_step, model, optimizer, emb_weights)
    ...
    with writer.as_default():
        for epoch in range(cfg['initial_epoch'], cfg['epochs']):
            ...
            if epoch % cfg['eval_every'] == 0 or epoch == epochs-1:
                t1 = time.time()
                accuracy, val, far, frr = lfw_evaluator.evaluate(predict_fn=predict_fn)
                ...
                lfw.save_lfw_result(lfw_results_file, accuracy, val, far, frr)

            writer.flush()
            try:

                # FIXME: For some reason, CheckpointManager throws the following error when saving model checkpoint
                ...
                # After some research, it is caused by insufficient memory.
                # The workaround here is to wrap the statement in try-catch statement to avoid failure.
                # I believe it's Tensorflow bug. Will post on Github/Stackoverflow later.

                # save_path = ckpt.save(cfg['ckpt_dir'])
...
def main():
    ...
    train_gen = TFRecordDataGenerator(cfg['data_dir'], batch_size=batch_size)
    ...
    with strategy.scope():
        ...
        train(cfg, model, emb_weights, optimizer, learning_rate, global_step, train_dist_ds, strategy, lfw_evaluator)
